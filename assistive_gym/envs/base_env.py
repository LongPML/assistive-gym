...
class BaseEnv:
    def __init__(
        self, time_step=0.02, frame_skip=5, render=True, gravity=-9.81, seed=1001
    ):
        ...
        self.human_creation = HumanCreation(
            self.id, np_random=self.np_random, cloth=False
        )
    ...
    def take_step(self, actions, gains=0.05, forces=1, action_multiplier=0.05):
        ...
        if self.action_space is None:
            self.action_space = spaces.Box(
                low=-np.ones(
                    np.sum([len(a.controllable_joint_indices) for a in self.agents])
                ),
                high=np.ones(
                    np.sum([len(a.controllable_joint_indices) for a in self.agents])
                ),
        ...
        for i, agent in enumerate(self.agents):
            ...
            for _ in range(self.frame_skip):
                action[
                    agent_joint_angles + action < agent.controllable_joint_lower_limits
                ] = 0
                action[
                    agent_joint_angles + action > agent.controllable_joint_upper_limits
                ] = 0
