...
class DressingEnv(AssistiveEnv):
    ...
    def step(self, action):
        ...
        preferences_score = self.human_preferences(
            ...
            dressing_forces=self.cloth_forces,
        )
    ...
    def _get_obs(self, agent=None):
        ...
        robot_joint_angles = (np.array(robot_joint_angles) + np.pi) % (
            2 * np.pi
        ) - np.pi
        if self.robot.mobile:
            # Don't include joint angles for the wheels
            robot_joint_angles = robot_joint_angles[
                len(self.robot.wheel_joint_indices) :
            ]
        shoulder_pos = self.human.get_pos_orient(self.human.left_shoulder)[0]
        elbow_pos = self.human.get_pos_orient(self.human.left_elbow)[0]
        wrist_pos = self.human.get_pos_orient(self.human.left_wrist)[0]
        ...
        self.robot_force_on_human = np.sum(
        ...
        if self.human.controllable:
            ...
            human_obs = np.concatenate(
                [
                    ...
                    elbow_pos_human,
                    wrist_pos_human,
                    [self.cloth_force_sum, self.robot_force_on_human],
                ]
            ).ravel()
            if agent == "human":
                return human_obs
    ...
    def reset(self):
        ...
        joints_positions = [
            ...
            (self.human.j_left_shoulder_x, -45),
        ...
        self.human.setup_joints(
            ...
            reactive_force=None if self.human.controllable else 1,
            reactive_gain=0.01,
        )
        ...
        target_ee_pos = np.array([0.45, -0.3, 1]) + self.np_random.uniform(
            -0.05, 0.05, size=3
        )
        target_ee_orient = self.get_quaternion(
            self.robot.toc_ee_orient_rpy[self.task][0]
        )
        target_ee_orient_shoulder = self.get_quaternion(
            self.robot.toc_ee_orient_rpy[self.task][-1]
        )
        ...
        self.cloth_attachment = self.create_sphere(
            radius=0.0001,
            mass=0,
        ...
        self.cloth = p.loadCloth(
            ...
            scale=1.4,
            mass=0.16,
            position=np.array([0.02, -0.38, 0.84]) + self.cloth_offset / 1.4,
            orientation=self.get_quaternion([0, 0, np.pi]),
            bodyAnchorId=self.cloth_attachment.body,
            anchors=[2086, 2087, 2088, 2041],
    ...
    def update_targets(self):
        ...
        # Force the end effector attachment to stay at the end effector
        self.cloth_attachment.set_base_pos_orient(
