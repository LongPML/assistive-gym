...
class FeedingMeshEnv(FeedingEnv):
    def __init__(self, robot, human):
        ...
        super(FeedingEnv, self).__init__(
            robot=robot,
            human=human,
            task="feeding",
            obs_robot_len=(
    ...
    def reset(self):
        ...
        joint_angles += [
            ...
            for j in (
                self.human.j_waist_x,
                self.human.j_waist_y,
                self.human.j_waist_z,
                self.human.j_lower_neck_x,
                self.human.j_lower_neck_y,
                self.human.j_lower_neck_z,
                self.human.j_upper_neck_x,
                self.human.j_upper_neck_y,
        ...
        self.human.set_base_pos_orient(
            self.furniture.get_base_pos_orient()[0]
            + chair_seat_position
        ...
        target_ee_orient = np.array(
            p.getQuaternionFromEuler(
                np.array(self.robot.toc_ee_orient_rpy[self.task]),
                physicsClientId=self.id,
            )
        )
        while True:
            ...
            _, _, _, _, dists_human = self.robot.get_closest_points(
                self.human, distance=0
            )
            _, _, _, _, dists_table = self.robot.get_closest_points(
                self.table, distance=0
            )
            _, _, _, _, dists_tool = self.tool.get_closest_points(
                self.human, distance=0
            )
        ...
        colors = [
            [60.0 / 256.0, 186.0 / 256.0, 84.0 / 256.0, 1],
            [244.0 / 256.0, 194.0 / 256.0, 13.0 / 256.0, 1],
            [219.0 / 256.0, 50.0 / 256.0, 54.0 / 256.0, 1],
            [72.0 / 256.0, 133.0 / 256.0, 237.0 / 256.0, 1],
        ]
    ...
    def generate_target(self):
        ...
        self.target = self.create_sphere(
            radius=0.01, mass=0.0, pos=mouth_pos, collision=False, rgba=[0, 1, 0, 1]
        )
