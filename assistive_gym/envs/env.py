...
from .agents import agent, furniture, human, panda, robot, tool
...
class AssistiveEnv(gym.Env):
    def __init__(
        ...
        self.human_creation = HumanCreation(
            self.id, np_random=self.np_random, cloth=("dressing" in task)
        )
        self.human_limits_model = load_model(
            os.path.join(self.directory, "realistic_arm_limits_model.h5")
        )
        self.action_robot_len = (
            len(robot.controllable_joint_indices) if robot is not None else 0
        )
        self.action_human_len = (
            len(human.controllable_joint_indices)
            if human is not None and human.controllable
            else 0
        )
        ...
        self.obs_human_len = (
            obs_human_len if human is not None and human.controllable else 0
        )
        self.observation_space = spaces.Box(
            low=np.array(
                [-1000000000.0] * (self.obs_robot_len + self.obs_human_len),
                dtype=np.float32,
            ),
            high=np.array(
                [1000000000.0] * (self.obs_robot_len + self.obs_human_len),
        ...
        self.action_space_robot = spaces.Box(
            low=np.array([-1.0] * self.action_robot_len, dtype=np.float32),
            high=np.array([1.0] * self.action_robot_len, dtype=np.float32),
            dtype=np.float32,
        )
        self.action_space_human = spaces.Box(
            low=np.array([-1.0] * self.action_human_len, dtype=np.float32),
    ...
    def reset(self):
        ...
        p.configureDebugVisualizer(
            p.COV_ENABLE_MOUSE_PICKING, 0, physicsClientId=self.id
        )
    ...
    def build_assistive_env(
        ...
        plane = p.loadURDF(
            os.path.join(self.directory, "plane", "plane.urdf"), physicsClientId=self.id
        )
        ...
        if self.robot is not None:
            self.robot.init(
                ...
                self.id,
                self.np_random,
                fixed_base=not self.robot.mobile,
            )
        ...
        if furniture_type is not None:
            self.furniture.init(
                ...
                self.id,
                self.np_random,
                wheelchair_mounted=self.robot.wheelchair_mounted
                if self.robot is not None
                else False,
            )
    ...
    def init_env_variables(self, reset=False):
        if len(self.action_space.low) <= 1 or reset:
            ...
            self.observation_space.__init__(
                low=-np.ones(obs_len, dtype=np.float32) * 1000000000,
                high=np.ones(obs_len, dtype=np.float32) * 1000000000,
                dtype=np.float32,
            )
            ...
            self.action_human_len = (
                len(self.human.controllable_joint_indices)
    ...
    def update_action_space(self):
        action_len = np.sum(
            [
                len(a.controllable_joint_indices)
                for a in self.agents
                if not isinstance(a, Human) or a.controllable
            ]
        )
        self.action_space.__init__(
            low=-np.ones(action_len, dtype=np.float32),
            high=np.ones(action_len, dtype=np.float32),
            dtype=np.float32,
        )
    ...
    def take_step(
        self, actions, gains=None, forces=None, action_multiplier=0.05, step_sim=True
    ):
        if gains is None:
        ...
        self.forces = []
        actions = np.clip(
            actions, a_min=self.action_space.low, a_max=self.action_space.high
        )
        ...
        for i, agent in enumerate(self.agents):
            ...
            if needs_action:
                ...
                action = np.copy(
                    actions[action_index : action_index + agent_action_len]
                )
                ...
                if len(action) != agent_action_len:
                    print(
                        "Received agent actions of length %d does not match expected action length of %d"
                        % (len(action), agent_action_len)
                    )
                    exit()
            # Append the new action to the current measured joint angles
            agent_joint_angles = agent.get_joint_angles(
            ...
            for _ in range(self.frame_skip):
                if needs_action:
                    below_lower_limits = (
                        ...
                        < agent.controllable_joint_lower_limits
                    )
                    above_upper_limits = (
                        agent_joint_angles + action
                        > agent.controllable_joint_upper_limits
                    )
                    action[below_lower_limits] = 0
                    action[above_upper_limits] = 0
                    agent_joint_angles[
                        below_lower_limits
                    ] = agent.controllable_joint_lower_limits[below_lower_limits]
                    agent_joint_angles[
                        above_upper_limits
                    ] = agent.controllable_joint_upper_limits[above_upper_limits]
            ...
            if isinstance(agent, Robot) and agent.action_duplication is not None:
                agent_joint_angles = np.concatenate(
                    [
                        [a] * d
                        for a, d in zip(
                            agent_joint_angles, self.robot.action_duplication
                        )
                    ]
                )
                agent.control(
                    agent.all_controllable_joints,
                    agent_joint_angles,
                    agent.gains,
                    agent.forces,
                )
    ...
    def human_preferences(
        ...
        reward_high_target_forces = (
            0 if tool_force_at_target < 10 else -tool_force_at_target
        )
        ...
        reward_food_velocities = (
            0 if len(food_mouth_velocities) == 0 else -np.sum(food_mouth_velocities)
        )
        ...
        if self.task == "arm_manipulation":
            tool_right_contact_points = len(
                self.tool_right.get_closest_points(self.human, distance=0.01)[-1]
            )
            tool_left_contact_points = len(
                self.tool_left.get_closest_points(self.human, distance=0.01)[-1]
            )
            tool_right_pressure = (
                0
                if tool_right_contact_points <= 0
                else (
            ...
            tool_left_pressure = (
                0
                if tool_left_contact_points <= 0
                else (
                    arm_manipulation_tool_forces_on_human[1] / tool_left_contact_points
                )
            )

            reward_arm_manipulation_tool_pressures = -(
                tool_right_pressure + tool_left_pressure
            )
            reward_force_nontarget = -(
                arm_manipulation_total_force_on_human
                - np.sum(arm_manipulation_tool_forces_on_human)
            )
        else:
        ...
        return (
            ...
            + self.C_fd * reward_food_hit_human
            + self.C_fdv * reward_food_velocities
    ...
    def init_robot_pose(
        ...
        for _ in range(max_iterations):
            if self.robot.mobile:
                ...
                base_position, _, _ = self.robot.position_robot_toc(
                    self.task,
                    arm,
                    start_pos_orient,
                    target_pos_orients,
    ...
    def get_euler(self, quaternion):
        return np.array(
            p.getEulerFromQuaternion(np.array(quaternion), physicsClientId=self.id)
        )

    def get_quaternion(self, euler):
        return np.array(
            p.getQuaternionFromEuler(np.array(euler), physicsClientId=self.id)
        )
    ...
    def setup_camera(
        ...
        self.view_matrix = p.computeViewMatrix(
            camera_eye, camera_target, [0, 0, 1], physicsClientId=self.id
        )
        self.projection_matrix = p.computeProjectionMatrixFOV(
            fov, camera_width / camera_height, 0.01, 100, physicsClientId=self.id
        )

    def setup_camera_rpy(
        self,
        camera_target=[-0.2, 0, 0.75],
        distance=1.5,
        rpy=[0, -35, 40],
    ...
    def create_sphere(
        ...
        sphere_collision = (
            p.createCollisionShape(
                shapeType=p.GEOM_SPHERE, radius=radius, physicsClientId=self.id
            )
            if collision
            else -1
        )
    ...
    def create_spheres(
        ...
        spheres = []
        for body in list(
            range(last_sphere_id - len(batch_positions) + 1, last_sphere_id + 1)
        ):
            sphere = Agent()
            sphere.init(body, self.id, self.np_random, indices=-1)
