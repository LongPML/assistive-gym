...
class Util:
        ...
        if enableGPU:
            ...
            if egl:
                eglPluginId = p.loadPlugin(
                    egl.get_filename(), "_eglRendererPlugin", physicsClientId=self.id
                )
            else:
    ...
    def point_on_capsule(self, p1, p2, radius, theta_range=(0, np.pi * 2)):
        ...
        point = (
            p1
            + random_length * axis_vector
            + radius * np.cos(theta) * ortho_vector
            + radius * np.sin(theta) * normal_vector
        )
        return point
    ...
    def capsule_points(self, p1, p2, radius, distance_between_points=0.05):
        ...
        section_positions = [
            (p2 - p1) / (sections + 1) * (i + 1) for i in range(sections)
        ]
    ...
    def line_intersects_triangle(self, p0, p1, p2, q0, q1):
        ...
        signed_volume = lambda a, b, c, d: (1.0 / 6.0) * np.dot(
            np.cross(b - a, c - a), d - a
        )
        if np.sign(signed_volume(q0, p0, p1, p2)) != np.sign(
            signed_volume(q1, p0, p1, p2)
        ):
            if (
                np.sign(signed_volume(q0, q1, p0, p1))
                == np.sign(signed_volume(q0, q1, p1, p2))
                == np.sign(signed_volume(q0, q1, p2, p0))
            ):
                return True
        return False

    def sleeve_on_arm_reward(
        self,
        triangle1_points,
        triangle2_points,
        shoulder_pos,
        ...
        wrist_pos, elbow_pos, shoulder_pos = (
            np.array(wrist_pos),
            np.array(elbow_pos),
        ...
        hand_end_pos = (
            wrist_pos
            + (wrist_pos - elbow_pos)
            / np.linalg.norm(wrist_pos - elbow_pos)
            * hand_radius
            * 2
        )
        elbow_end_pos = (
        ...
        shoulder_end_pos = (
            ...
            / np.linalg.norm(shoulder_pos - elbow_pos)
            * shoulder_radius
        )
        ...
        binormal_forearm_points = np.dot(
            binormal_forearm, (all_points - hand_end_pos).T
        )
        points_above_below_forearm = (
            np.any(tangent_forearm_points > 0)
            and np.any(tangent_forearm_points < 0)
            and np.any(binormal_forearm_points > 0)
            and np.any(binormal_forearm_points < 0)
        )
        ...
        tangent_upperarm_points = np.dot(
            tangent_upperarm, (all_points - shoulder_end_pos).T
        )
        binormal_upperarm_points = np.dot(
            binormal_upperarm, (all_points - shoulder_end_pos).T
        )
        points_above_below_upperarm = (
            np.any(tangent_upperarm_points > 0)
            and np.any(tangent_upperarm_points < 0)
            and np.any(binormal_upperarm_points > 0)
            and np.any(binormal_upperarm_points < 0)
        )

        # Check that the arm line segment intersects two different triangles defined by points around the sleeve.
        # https://stackoverflow.com/questions/42740765/intersection-between-line-and-triangle-in-3d
        forearm_intersects_triangle1 = self.line_intersects_triangle(
            triangle1_points[0],
            triangle1_points[1],
            triangle1_points[2],
            hand_end_pos,
        ...
        forearm_intersects_triangle2 = self.line_intersects_triangle(
            triangle2_points[0],
            triangle2_points[1],
            triangle2_points[2],
            hand_end_pos,
        ...
        upperarm_intersects_triangle1 = self.line_intersects_triangle(
            triangle1_points[0],
            triangle1_points[1],
            triangle1_points[2],
            elbow_end_pos,
        ...
        upperarm_intersects_triangle2 = self.line_intersects_triangle(
            triangle2_points[0],
            triangle2_points[1],
            triangle2_points[2],
            elbow_end_pos,
        ...
        return (
            ...
            np.linalg.norm(hand_end_pos - elbow_end_pos),
            np.linalg.norm(elbow_pos - shoulder_pos),
        )
