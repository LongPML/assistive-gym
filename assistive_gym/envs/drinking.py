...
class DrinkingEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(DrinkingEnv, self).__init__(
            robot=robot,
            human=human,
            task="drinking",
            obs_robot_len=(
    ...
    def step(self, action):
        ...
        (
            ...
            water_hit_human_reward,
        ) = self.get_water_rewards()
        ...
        preferences_score = self.human_preferences(
            ...
            tool_force_at_target=self.cup_force_on_human,
        ...
        info = {
            ...
            "task_success": int(
                self.task_success
                >= self.total_water_count * self.config("task_success_threshold")
            ),
    ...
    def get_water_rewards(self):
        ...
        bottom_center_pos = np.array(
            p.multiplyTransforms(
                cup_pos,
                cup_orient,
                self.cup_bottom_center_offset,
                [0, 0, 0, 1],
                physicsClientId=self.id,
            )[0]
        )
        ...
        for w in self.waters:
            ...
            if not self.util.points_in_cylinder(
                top_center_pos, bottom_center_pos, 0.05, np.array(water_pos)
            ):
        ...
        self.waters_active = [
            w for w in self.waters_active if w not in waters_active_to_remove
        ]
        return water_reward, water_mouth_velocities, water_hit_human_reward

    def _get_obs(self, agent=None):
        cup_pos, cup_orient = self.tool.get_base_pos_orient()
        cup_pos_real, cup_orient_real = self.robot.convert_to_realworld(
            cup_pos, cup_orient
        )
        ...
        robot_obs = np.concatenate(
            [
                cup_pos_real,
                cup_orient_real,
                cup_pos_real - target_pos_real,
                robot_joint_angles,
                head_pos_real,
                head_orient_real,
                [self.cup_force_on_human],
            ]
        ).ravel()
        if agent == "robot":
            return robot_obs
        if self.human.controllable:
            ...
            cup_pos_human, cup_orient_human = self.human.convert_to_realworld(
                cup_pos, cup_orient
            )
            ...
            human_obs = np.concatenate(
                [
                    cup_pos_human,
                    cup_orient_human,
                    cup_pos_human - target_pos_human,
                    human_joint_angles,
                    head_pos_human,
                    head_orient_human,
                    [self.robot_force_on_human, self.cup_force_on_human],
                ]
            ).ravel()
            if agent == "human":
                return human_obs
    ...
    def reset(self):
        ...
        target_ee_pos = np.array([-0.2, -0.5, 1.1]) + self.np_random.uniform(
            -0.05, 0.05, size=3
        )
        ...
        p.setPhysicsEngineParameter(
            numSubSteps=4, numSolverIterations=10, physicsClientId=self.id
        )
        ...
        self.waters = self.create_spheres(
            radius=water_radius,
            mass=water_mass,
        ...
        for w in self.waters:
            p.changeVisualShape(
                w.body, -1, rgbaColor=[0.25, 0.5, 1, 1], physicsClientId=self.id
            )
