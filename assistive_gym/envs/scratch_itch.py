...
class ScratchItchEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(ScratchItchEnv, self).__init__(
            robot=robot,
            human=human,
            task="scratch_itch",
            obs_robot_len=(
            ...
            obs_human_len=(24 + len(human.controllable_joint_indices)),
        )
    ...
    def step(self, action):
        ...
        reward_distance = -np.linalg.norm(
            self.target_pos - tool_pos
        )  # Penalize distances away from target
        reward_action = -np.linalg.norm(action)  # Penalize actions
        reward_force_scratch = 0.0  # Reward force near the target
        if (
            self.target_contact_pos is not None
            and np.linalg.norm(self.target_contact_pos - self.prev_target_contact_pos)
            > 0.01
            and self.tool_force_at_target < 10
        ):
        ...
        if self.gui and self.tool_force_at_target > 0:
            print(
                "Task success:",
                self.task_success,
                "Tool force at target:",
                self.tool_force_at_target,
                reward_force_scratch,
            )
    ...
    def get_total_force(self):
        ...
        for linkA, linkB, posA, posB, force in zip(
            *self.tool.get_contact_points(self.human)
        ):
        ...
        return (
            ...
            None if target_contact_pos is None else np.array(target_contact_pos),
        )

    def _get_obs(self, agent=None):
        tool_pos, tool_orient = self.tool.get_pos_orient(1)
        tool_pos_real, tool_orient_real = self.robot.convert_to_realworld(
        ...
        (
            ...
            self.tool_force,
            self.tool_force_at_target,
            self.target_contact_pos,
        ) = self.get_total_force()
        robot_obs = np.concatenate(
            [
                tool_pos_real,
                tool_orient_real,
                tool_pos_real - target_pos_real,
                target_pos_real,
                robot_joint_angles,
                shoulder_pos_real,
                elbow_pos_real,
        ...
        if self.human.controllable:
            ...
            tool_pos_human, tool_orient_human = self.human.convert_to_realworld(
                tool_pos, tool_orient
            )
            ...
            human_obs = np.concatenate(
                [
                    tool_pos_human,
                    tool_orient_human,
                    tool_pos_human - target_pos_human,
                    target_pos_human,
                    human_joint_angles,
                    shoulder_pos_human,
                    elbow_pos_human,
                    wrist_pos_human,
                    [self.total_force_on_human, self.tool_force_at_target],
                ]
            ).ravel()
            if agent == "human":
                return human_obs
    ...
    def reset(self):
        ...
        joints_positions = [
            (self.human.j_right_shoulder_x, 30),
        ...
        target_ee_pos = np.array([-0.6, 0, 0.8]) + self.np_random.uniform(
            -0.05, 0.05, size=3
        )
    ...
    def generate_target(self):
        ...
        if self.human.gender == "male":
            self.limb, length, radius = [
                [self.human.right_shoulder, 0.279, 0.043],
                [self.human.right_elbow, 0.257, 0.033],
            ][self.np_random.randint(2)]
        else:
            self.limb, length, radius = [
                [self.human.right_shoulder, 0.264, 0.0355],
                [self.human.right_elbow, 0.234, 0.027],
            ][self.np_random.randint(2)]
