...
class Jaco(Robot):
    def __init__(self, controllable_joints="right"):
        ...
        gripper_pos = {
            "scratch_itch": [1] * 3,  # Gripper open position for holding tools
            "feeding": [1.33] * 3,
            "drinking": [0.63] * 3,
            "bed_bathing": [1.1] * 3,
            "dressing": [1.33] * 3,
        ...
        pos = [-0.35, -0.3, 0.3]
        toc_base_pos_offset = {
            "scratch_itch": pos,  # Robot base offset before TOC base pose optimization
            "feeding": pos,
            "drinking": pos,
    ...
    def init(self, directory, id, np_random, fixed_base=True):
        self.body = p.loadURDF(
            os.path.join(directory, "jaco", "j2s7s300_gym.urdf"),
