...
class Stretch(Robot):
    def __init__(self, controllable_joints="right"):
        ...
        gripper_pos = {
            "scratch_itch": [0.1, 0.1],  # Gripper open position for holding tools
            "feeding": [0, 0],
            "drinking": [0.2, 0.2],
            "bed_bathing": [0.1, 0.1],
            "dressing": [0, 0],
        ...
        toc_ee_orient_rpy = {
            "scratch_itch": [0, 0, np.pi / 2.0],  # Initial end effector orientation
        ...
        self.action_duplication = (
            [1, 1, 1, 4, 1] if "wheel" in controllable_joints else [1, 4, 1]
        )  # The urdf models the prismatic arm as multiple joints, but we want only 1 action to control all of them.
        ...
        self.all_controllable_joints = (
            [0, 1, 3, 5, 6, 7, 8, 9]
            if "wheel" in controllable_joints
            else [3, 5, 6, 7, 8, 9]
        )
        ...
        super(Stretch, self).__init__(
            ...
            wheelchair_mounted,
            half_range=False,
            action_duplication=self.action_duplication,
    ...
    def init(self, directory, id, np_random, fixed_base=False):
        ...
        self.body = p.loadURDF(
            os.path.join(directory, "stretch", "stretch_uncalibrated.urdf"),
            useFixedBase=False,
            basePosition=[-1, -1, 0.09],
            physicsClientId=id,
        )
