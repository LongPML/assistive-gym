...
class Robot(Agent):
    def __init__(
        ...
        tool_pos_offset,
        tool_orient_offset,
        right_gripper_collision_indices,
        left_gripper_collision_indices,
        toc_base_pos_offset,
        toc_ee_orient_rpy,
        wheelchair_mounted,
        half_range=False,
        controllable_joint_indices=None,
        action_duplication=None,
        action_multiplier=1,
        flags=None,
    ):
        self.controllable_joints = (
            controllable_joints if controllable_joint_indices is None else ""
        )
        self.right_arm_joint_indices = (
            right_arm_joint_indices  # Controllable arm joints
        )
        ...
        if controllable_joint_indices is not None:
            ...
            self.controllable_joint_indices = (
                self.wheel_joint_indices if self.mobile else []
            )
            self.controllable_joint_indices = self.controllable_joint_indices + (
                self.right_arm_joint_indices
                if "right" in controllable_joints
                else self.left_arm_joint_indices
                if "left" in controllable_joints
                else self.right_arm_joint_indices + self.left_arm_joint_indices
            )
        self.right_end_effector = (
            right_end_effector  # Used to get the pose of the end effector
        )
        self.left_end_effector = (
            left_end_effector  # Used to get the pose of the end effector
        )
        ...
        self.toc_base_pos_offset = (
            toc_base_pos_offset  # Robot base offset before TOC base pose optimization
        )
    ...
    def enable_wheels(self):
        ...
        self.controllable_joint_indices = self.wheel_joint_indices + (
            self.right_arm_joint_indices
            if "right" in self.controllable_joints
            else self.left_arm_joint_indices
            if "left" in self.controllable_joints
            else self.right_arm_joint_indices + self.left_arm_joint_indices
        )
    ...
    def init(self, body, id, np_random):
        ...
        if self.mobile:
            self.controllable_joint_lower_limits[
                : len(self.wheel_joint_indices)
            ] = -np.inf
            self.controllable_joint_upper_limits[
                : len(self.wheel_joint_indices)
            ] = np.inf
        self.right_arm_lower_limits = [
            self.lower_limits[i] for i in self.right_arm_joint_indices
        ]
        self.right_arm_upper_limits = [
            self.upper_limits[i] for i in self.right_arm_joint_indices
        ]
        self.left_arm_lower_limits = [
            self.lower_limits[i] for i in self.left_arm_joint_indices
        ]
        self.left_arm_upper_limits = [
            self.upper_limits[i] for i in self.left_arm_joint_indices
        ]
        self.joint_max_forces = self.get_joint_max_force(
            self.controllable_joint_indices
        )
    ...
    def set_gripper_open_position(
        self, indices, positions, set_instantly=False, force=500
    ):
    ...
    def ik_random_restarts(
        ...
        for r in range(max_ik_random_restarts):
            target_joint_angles = self.ik(
                self.right_end_effector if right else self.left_end_effector,
                target_pos,
                target_orient,
                ik_indices=self.right_arm_ik_indices
                if right
                else self.left_arm_ik_indices,
                max_iterations=max_iterations,
                half_range=self.half_range,
                randomize_limits=(randomize_limits and r >= 10),
            )
            self.set_joint_angles(
                self.right_arm_joint_indices if right else self.left_arm_joint_indices,
                target_joint_angles,
            )
            ...
            if np.linalg.norm(
                target_pos - np.array(gripper_pos)
            ) < success_threshold and (
                target_orient is None
                or np.linalg.norm(target_orient - np.array(gripper_orient))
                < success_threshold
                or np.isclose(
                    np.linalg.norm(target_orient - np.array(gripper_orient)),
                    2,
                    atol=success_threshold,
                )
            ):
                # if step_sim:
            ...
            if (
                best_ik_angles is None
                or np.linalg.norm(target_pos - np.array(gripper_pos)) < best_ik_distance
            ):
    ...
    def position_robot_toc(
        ...
        while iteration < attempts or best_position is None:
            ...
            random_pos = np.array(
                [
                    self.np_random.uniform(
                        -random_position if right_side else 0,
                        0 if right_side else random_position,
                    ),
                    self.np_random.uniform(-random_position, random_position),
                    0,
                ]
            )
            random_orientation = self.get_quaternion(
                [
                    base_euler_orient[0],
                    base_euler_orient[1],
                    base_euler_orient[2]
            ...
            self.set_base_pos_orient(
                np.array([-0.85, -0.4, 0])
                + self.toc_base_pos_offset[task]
                + random_pos,
                random_orientation,
            )
            ...
            for i, arm in enumerate(arms):
                ...
                ik_indices = (
                    self.right_arm_ik_indices if right else self.left_arm_ik_indices
                )
                lower_limits = (
                    self.right_arm_lower_limits if right else self.left_arm_lower_limits
                )
                upper_limits = (
                    self.right_arm_upper_limits if right else self.left_arm_upper_limits
                )
                for j, (target_pos, target_orient) in enumerate(
                    start_pos_orient[i] + target_pos_orients[i]
                ):
                    best_jlwki = None
                    ...
                    for k in range(jlwki_restarts):
                        ...
                        human.set_joint_angles(
                            human.controllable_joint_indices, human_angles
                        )
                        # Find IK solution
                        success, joint_positions_q_star = self.ik_random_restarts(
                            right,
                            target_pos,
                            target_orient,
                        ...
                        joint_limit_weight = self.joint_limited_weighting(
                            joint_positions_q_star, lower_limits, upper_limits
                        )
                        # Joint-limited-weighted kinematic isotropy (JLWKI)
                        det = max(
                            np.linalg.det(
                                np.matmul(np.matmul(J, joint_limit_weight), J.T)
                            ),
                            0,
                        )
                        jlwki = np.power(det, 1.0 / a) / (
                            np.trace(np.matmul(np.matmul(J, joint_limit_weight), J.T))
                            / a
                        )
            ...
            if num_goals_reached > 0:
                if (
                    ...
                    or num_goals_reached > best_num_goals_reached
                    or (
                        num_goals_reached == best_num_goals_reached
                        and manipulability > best_manipulability
                    )
                ):
        ...
        p.resetBasePositionAndOrientation(
            self.body,
            np.array([-0.85, -0.4, 0])
            + np.array(self.toc_base_pos_offset[task])
            + best_position,
            best_orientation,
            physicsClientId=self.id,
        )
    ...
    def joint_limited_weighting(self, q, lower_limits, upper_limits):
        ...
        for qi, l, u in zip(q, lower_limits, upper_limits):
            qr = 0.5 * (u - l)
            weights.append(
                1.0 - np.power(phi, (qr - np.abs(qr - qi + l)) / (lam * qr) + 1)
            )
            if weights[-1] < 0.001:
                weights[-1] = 0.001
        # Joint-limited-weighting
