...
class PR2(Robot):
    def __init__(self, controllable_joints="right"):
        ...
        right_gripper_collision_indices = list(
            range(49, 64)
        )  # Used to disable collision between gripper and tools
        left_gripper_collision_indices = list(
            range(71, 86)
        )  # Used to disable collision between gripper and tools
        gripper_pos = {
            "scratch_itch": [0.25] * 4,  # Gripper open position for holding tools
            "feeding": [0.03] * 4,
            "drinking": [0.45] * 4,
            "bed_bathing": [0.2] * 4,
            "dressing": [0] * 4,
        ...
        toc_ee_orient_rpy = {
            "scratch_itch": [0, 0, 0],  # Initial end effector orientation
    ...
    def init(self, directory, id, np_random, fixed_base=True):
        self.body = p.loadURDF(
            os.path.join(directory, "PR2", "pr2_no_torso_lift_tall.urdf"),
        ...
        for i in [19, 42, 64]:
            p.changeVisualShape(
                self.body, i, rgbaColor=[1.0, 1.0, 1.0, 1.0], physicsClientId=id
            )
        for i in [43, 46, 49, 58, 60, 65, 68, 71, 80, 82]:
            p.changeVisualShape(
                self.body, i, rgbaColor=[0.4, 0.4, 0.4, 1.0], physicsClientId=id
            )
        for i in [45, 51, 67, 73]:
            p.changeVisualShape(
                self.body, i, rgbaColor=[0.7, 0.7, 0.7, 1.0], physicsClientId=id
            )
        p.changeVisualShape(
            self.body, 20, rgbaColor=[0.8, 0.8, 0.8, 1.0], physicsClientId=id
        )
        p.changeVisualShape(
            self.body, 40, rgbaColor=[0.6, 0.6, 0.6, 1.0], physicsClientId=id
        )
    ...
    def reset_joints(self):
        ...
        self.set_joint_angles(
            self.right_arm_joint_indices, [-1.75, 1.25, -1.5, -0.5, -1, 0, -1]
        )
        self.set_joint_angles(
            self.left_arm_joint_indices, [1.75, 1.25, 1.5, -0.5, 1, 0, 1]
        )
