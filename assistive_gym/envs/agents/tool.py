...
class Tool(Agent):
    ...
    def init(
        ...
        if task == "scratch_itch":
            tool = p.loadURDF(
                os.path.join(directory, "scratcher", "tool_scratch.urdf"),
            ...
            tool = p.loadURDF(
                os.path.join(directory, "bed_bathing", "wiper.urdf"),
                basePosition=transform_pos,
                baseOrientation=transform_orient,
                physicsClientId=id,
            )
            ...
            if task == "drinking":
                visual_filename = os.path.join(
                    directory, "dinnerware", "plastic_coffee_cup.obj"
                )
                collision_filename = os.path.join(
                    directory, "dinnerware", "plastic_coffee_cup_vhacd.obj"
                )
            elif task == "feeding":
                visual_filename = os.path.join(directory, "dinnerware", "spoon.obj")
                collision_filename = os.path.join(
                    directory, "dinnerware", "spoon_vhacd.obj"
                )
            elif task == "arm_manipulation":
                visual_filename = os.path.join(
                    directory, "arm_manipulation", "arm_manipulation_scooper.obj"
                )
                collision_filename = os.path.join(
                    directory, "arm_manipulation", "arm_manipulation_scooper_vhacd.obj"
                )
        ...
        if robot is not None:
            ...
            for j in (
                robot.right_gripper_collision_indices
                if right
                else robot.left_gripper_collision_indices
            ):
                for tj in self.all_joint_indices + [self.base]:
                    p.setCollisionFilterPair(
                        robot.body, self.body, j, tj, False, physicsClientId=id
                    )
            # Create constraint that keeps the tool in the gripper
            constraint = p.createConstraint(
                robot.body,
                robot.right_tool_joint if right else robot.left_tool_joint,
                self.body,
                -1,
                p.JOINT_FIXED,
                [0, 0, 0],
                parentFramePosition=self.pos_offset,
                childFramePosition=[0, 0, 0],
                parentFrameOrientation=self.orient_offset,
    ...
    def get_transform(self):
        if self.robot is not None:
            ...
            gripper_pos, gripper_orient = self.robot.get_pos_orient(
                ...
                if self.right
                else self.robot.left_tool_joint,
                center_of_mass=True,
            )
