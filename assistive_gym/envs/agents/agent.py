...
class Agent:
    ...
    def init(self, body, id, np_random, indices=None):
        ...
        if indices != -1:
            ...
            self.controllable_joint_lower_limits = np.array(
                [self.lower_limits[i] for i in self.controllable_joint_indices]
            )
            self.controllable_joint_upper_limits = np.array(
                [self.upper_limits[i] for i in self.controllable_joint_indices]
            )
    ...
    def get_joint_angles(self, indices=None):
        ...
        robot_joint_states = p.getJointStates(
            self.body, jointIndices=indices, physicsClientId=self.id
        )
    ...
    def get_pos_orient(self, link, center_of_mass=False, convert_to_realworld=False):
        ...
        if link == self.base:
            pos, orient = p.getBasePositionAndOrientation(
                self.body, physicsClientId=self.id
            )
        else:
            if not center_of_mass:
                pos, orient = p.getLinkState(
                    self.body,
                    link,
    ...
    def convert_to_realworld(self, pos, orient=[0, 0, 0, 1]):
        ...
        base_pos_inv, base_orient_inv = p.invertTransform(
            base_pos, base_orient, physicsClientId=self.id
        )
        real_pos, real_orient = p.multiplyTransforms(
            base_pos_inv,
            base_orient_inv,
            pos,
    ...
    def get_motor_joint_states(self, joints=None):
        ...
        joint_infos = [
            p.getJointInfo(self.body, i, physicsClientId=self.id)
            for i in (self.all_joint_indices if joints is None else joints)
        ]
        motor_states = [
            j for j, i in zip(joint_states, joint_infos) if i[2] != p.JOINT_FIXED
        ]
        motor_indices = [
            i[0] for j, i in zip(joint_states, joint_infos) if i[2] != p.JOINT_FIXED
        ]
    ...
    def get_joint_max_force(self, indices=None):
        if indices is None:
            indices = self.all_joint_indices
        joint_infos = [
            p.getJointInfo(self.body, i, physicsClientId=self.id) for i in indices
        ]
    ...
    def set_joint_angles(self, indices, angles, use_limits=True, velocities=0):
        for i, (j, a) in enumerate(zip(indices, angles)):
            p.resetJointState(
                self.body,
                jointIndex=j,
                targetValue=min(max(a, self.lower_limits[j]), self.upper_limits[j])
                if use_limits
                else a,
    ...
    def set_whole_body_frictions(
        self, lateral_friction=None, spinning_friction=None, rolling_friction=None
    ):
        self.set_frictions(
    ...
    def set_joint_stiffness(self, joint, stiffness):
        p.changeDynamics(
            self.body, joint, jointDamping=stiffness, physicsClientId=self.id
        )
    ...
    def enable_force_torque_sensor(self, joint):
        p.enableJointForceTorqueSensor(
            self.body, joint, enableSensor=True, physicsClientId=self.id
        )
        ...
        for j in indices:
            ...
            if lower_limit == 0 and upper_limit == -1:
                ...
                if joint_type != p.JOINT_FIXED:
                    ...
                    self.ik_joint_names.append(
                        [len(self.ik_joint_names)] + list(joint_info[:2])
                    )
        ...
        for j in indices:
            if joint_angles[j] < self.lower_limits[j]:
                p.resetJointState(
                    self.body,
                    jointIndex=j,
                    targetValue=self.lower_limits[j],
                    targetVelocity=0,
                    physicsClientId=self.id,
                )
                ...
                p.resetJointState(
                    self.body,
                    jointIndex=j,
                    targetValue=self.upper_limits[j],
                    targetVelocity=0,
                    physicsClientId=self.id,
                )
    ...
    def ik(
        ...
        ik_lower_limits = (
            self.ik_lower_limits
            if not randomize_limits
            else self.np_random.uniform(0, self.ik_lower_limits)
        )
        ik_upper_limits = (
            self.ik_upper_limits
            if not randomize_limits
            else self.np_random.uniform(0, self.ik_upper_limits)
        )
