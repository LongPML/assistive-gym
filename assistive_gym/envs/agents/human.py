...
class Human(Agent):
    ...
    def init(
        ...
        self.limit_scale = (
            1.0 if impairment != "limits" else np_random.uniform(0.5, 1.0)
        )
        self.strength = (
            1.0 if impairment != "weakness" else np_random.uniform(0.25, 1.0)
        )
        if self.impairment != "tremor":
            ...
            self.tremors = np_random.uniform(
                np.deg2rad(-20),
                np.deg2rad(20),
                size=len(self.controllable_joint_indices),
            )
        else:
            self.tremors = np_random.uniform(
                np.deg2rad(-10),
        ...
        # Initialize human
        self.body = human_creation.create_human(
            static=static_human_base,
        ...
        super(Human, self).init(
            self.body, id, np_random, self.controllable_joint_indices
        )

        # By default, initialize the person in the wheelchair
        self.set_base_pos_orient(
            [0, 0.03, 0.89 if self.gender == "male" else 0.86], [0, 0, 0, 1]
        )
    ...
    def setup_joints(
        ...
        for j in self.all_joint_indices:
            if use_static_joints and (
                j not in self.controllable_joint_indices
                or (
                    self.impairment != "tremor"
                    and reactive_force is None
                    and not self.controllable
                )
            ):
        ...
        self.target_joint_angles = self.get_joint_angles(
    ...
    def enforce_realistic_joint_limits(self):
        ...
        indices = (
            [
                self.j_right_shoulder_x,
                self.j_right_shoulder_y,
                self.j_right_shoulder_z,
                self.j_right_elbow,
            ]
            if right
            else [
                self.j_left_shoulder_x,
                self.j_left_shoulder_y,
                self.j_left_shoulder_z,
                self.j_left_elbow,
            ]
        )
