    ...
    def init(self, furniture_type, directory, id, np_random, wheelchair_mounted=False):
        if "wheelchair" in furniture_type:
            ...
            if "left" in furniture_type:
                ...
                left = True
            furniture = p.loadURDF(
                os.path.join(
                    directory,
                    furniture_type,
                    "wheelchair.urdf"
                    if not wheelchair_mounted
                    else (
                        "wheelchair_jaco.urdf"
                        if not left
                        else "wheelchair_jaco_left.urdf"
                    ),
                ),
            ...
            furniture = p.loadURDF(
                os.path.join(directory, "bed", "bed.urdf"),
            ...
            furniture = p.loadURDF(
                os.path.join(directory, "bed", "hospital_bed.urdf"),
                basePosition=[0, 0, 0],
                baseOrientation=[0, 0, 0, 1],
                physicsClientId=id,
            )
            ...
            furniture = p.loadURDF(
                os.path.join(directory, "table", "table_tall.urdf"),
                basePosition=[0.25, -1.0, 0],
                baseOrientation=[0, 0, 0, 1],
                physicsClientId=id,
            )
        elif furniture_type == "bowl":
            bowl_pos = np.array([-0.15, -0.65, 0.75]) + np.array(
                [np_random.uniform(-0.05, 0.05), np_random.uniform(-0.05, 0.05), 0]
            )
            furniture = p.loadURDF(
                os.path.join(directory, "dinnerware", "bowl.urdf"),
            ...
            furniture = p.loadURDF(
                os.path.join(directory, "nightstand", "nightstand.urdf"),
                basePosition=np.array([-0.9, 0.7, 0]),
                baseOrientation=[0, 0, 0, 1],
                physicsClientId=id,
            )
        else:
