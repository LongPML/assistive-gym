...
class Baxter(Robot):
    def __init__(self, controllable_joints="right"):
        ...
        gripper_pos = {
            "scratch_itch": [0.015, -0.015],  # Gripper open position for holding tools
            "feeding": [0, 0],
        ...
        toc_ee_orient_rpy = {
            "scratch_itch": [0, np.pi / 2.0, 0],  # Initial end effector orientation
    ...
    def init(self, directory, id, np_random, fixed_base=True):
        self.body = p.loadURDF(
            os.path.join(directory, "baxter", "baxter_custom.urdf"),
            useFixedBase=fixed_base,
            basePosition=[-1, -1, 0.925],
            physicsClientId=id,
        )
        ...
        for i in [20, 21, 23, 31, 32, 42, 43, 45, 53, 54]:
            p.changeVisualShape(
                self.body, i, rgbaColor=[1.0, 1.0, 1.0, 0.0], physicsClientId=id
            )
    ...
    def reset_joints(self):
        ...
        self.set_joint_angles(
            self.right_arm_joint_indices, [-0.75, 1, -0.5, 0.5, -1, -0.5, 0]
        )
        self.set_joint_angles(
            self.left_arm_joint_indices, [0.75, 1, 0.5, 0.5, 1, -0.5, 0]
        )
