...
class BedBathingEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(BedBathingEnv, self).__init__(
            robot=robot,
            human=human,
            task="bed_bathing",
            obs_robot_len=(
            ...
            *self.tool.get_contact_points(self.human)
            ...
            if linkA in [1]:
                ...
                for i, (target_pos_world, target) in enumerate(
                    zip(self.targets_pos_upperarm_world, self.targets_upperarm)
                ):
                ...
                self.targets_pos_on_upperarm = [
                    t
                    for i, t in enumerate(self.targets_pos_on_upperarm)
                    if i not in indices_to_delete
                ]
                self.targets_upperarm = [
                    t
                    for i, t in enumerate(self.targets_upperarm)
                    if i not in indices_to_delete
                ]
                self.targets_pos_upperarm_world = [
                    t
                    for i, t in enumerate(self.targets_pos_upperarm_world)
                    if i not in indices_to_delete
                ]

                indices_to_delete = []
                for i, (target_pos_world, target) in enumerate(
                    zip(self.targets_pos_forearm_world, self.targets_forearm)
                ):
                    if np.linalg.norm(posB - target_pos_world) < 0.025:
                        # The robot made contact with a point on the person's arm
                        new_contact_points += 1
                ...
                self.targets_pos_on_forearm = [
                    t
                    for i, t in enumerate(self.targets_pos_on_forearm)
                    if i not in indices_to_delete
                ]
                self.targets_forearm = [
                    t
                    for i, t in enumerate(self.targets_forearm)
                    if i not in indices_to_delete
                ]
                self.targets_pos_forearm_world = [
                    t
                    for i, t in enumerate(self.targets_pos_forearm_world)
                    if i not in indices_to_delete
                ]
        ...
        if self.human.controllable:
            ...
            human_obs = np.concatenate(
                [
                    ...
                    elbow_pos_human,
                    wrist_pos_human,
                    [self.total_force_on_human, self.tool_force_on_human],
                ]
            ).ravel()
            if agent == "human":
                return human_obs
    ...
    def reset(self):
        ...
        target_ee_pos = np.array([-0.6, 0.2, 1]) + self.np_random.uniform(
            -0.05, 0.05, size=3
        )
        ...
        if self.robot.wheelchair_mounted:
            ...
            self.nightstand.set_base_pos_orient(
                np.array([-0.9, 0.7, 0]) + base_position, [0, 0, 0, 1]
            )
