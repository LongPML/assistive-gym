...
class ArmManipulationEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(ArmManipulationEnv, self).__init__(
            robot=robot,
            human=human,
            task="arm_manipulation",
            obs_robot_len=(
                31
                + len(robot.controllable_joint_indices)
                - (len(robot.wheel_joint_indices) if robot.mobile else 0)
            ),
            obs_human_len=(32 + len(human.controllable_joint_indices)),
        )
    ...
    def step(self, action):
        ...
        preferences_score = self.human_preferences(
            ...
            arm_manipulation_tool_forces_on_human=[
                self.tool_right_force_on_human,
                self.tool_left_force_on_human,
            ],
            arm_manipulation_total_force_on_human=self.total_force_on_human,
        )
        ...
        reward_distance_robot_left = -np.linalg.norm(
            tool_left_pos - elbow_pos
        )  # Penalize distances away from human hand
        reward_distance_robot_right = -np.linalg.norm(
            tool_right_pos - wrist_pos
        )  # Penalize distances away from human hand
        reward_distance_human = -np.linalg.norm(
            elbow_pos - stomach_pos
        ) - np.linalg.norm(
            wrist_pos - waist_pos
        )  # Penalize distances between human hand and waist
        reward_action = -np.linalg.norm(action)  # Penalize actions

        if self.robot.has_single_arm:
            reward = (
                self.config("distance_human_weight") * reward_distance_human
                + 2
                * self.config("distance_end_effector_weight")
                * reward_distance_robot_left
    ...
    def get_total_force(self):
        ...
        tool_right_force_on_human = np.sum(
            self.tool_right.get_contact_points(self.human)[-1]
        )
        tool_left_force_on_human = np.sum(
            self.tool_left.get_contact_points(self.human)[-1]
        )
        total_force_on_human = (
            np.sum(self.robot.get_contact_points(self.human)[-1])
            + tool_right_force_on_human
            + tool_left_force_on_human
        )
        return (
            tool_right_force,
            tool_left_force,
    ...
    def _get_obs(self, agent=None):
        ...
        tool_right_pos_real, tool_right_orient_real = self.robot.convert_to_realworld(
            tool_right_pos, tool_right_orient
        )
        tool_left_pos_real, tool_left_orient_real = self.robot.convert_to_realworld(
            tool_left_pos, tool_left_orient
        )
        ...
        (
            self.tool_right_force,
            self.tool_left_force,
            self.tool_right_force_on_human,
            self.tool_left_force_on_human,
            self.total_force_on_human,
        ...
        robot_obs = np.concatenate(
            [
                tool_right_pos_real,
                tool_right_orient_real,
                tool_left_pos_real,
                tool_left_orient_real,
                robot_joint_angles,
                shoulder_pos_real,
                elbow_pos_real,
                wrist_pos_real,
                stomach_pos_real,
                waist_pos_real,
                [self.tool_left_force, self.tool_right_force],
            ]
        ).ravel()
        if agent == "robot":
            return robot_obs
        if self.human.controllable:
            ...
            (
                tool_right_pos_human,
                tool_right_orient_human,
            ) = self.human.convert_to_realworld(tool_right_pos, tool_right_orient)
            (
                tool_left_pos_human,
                tool_left_orient_human,
            ) = self.human.convert_to_realworld(tool_left_pos, tool_left_orient)
    ...
    def reset(self):
        ...
        self.build_assistive_env(
            "bed", fixed_human_base=False, human_impairment="no_tremor"
        )
        ...
        self.human.set_joint_angles(
            motor_indices, self.np_random.uniform(-0.1, 0.1, size=len(motor_indices))
        )
        ...
        joints_positions = [
            (self.human.j_right_shoulder_x, 60),
            (self.human.j_right_shoulder_y, -60),
            (self.human.j_right_elbow, 0),
        ]
        self.human.setup_joints(
            joints_positions, use_static_joints=True, reactive_force=0.01
        )
        self.human.set_mass(self.human.base, mass=0)
        self.human.set_base_velocity(
        ...
        if self.robot.has_single_arm:
            base_position = self.init_robot_pose(
                target_ee_right_pos,
                target_ee_orient,
                [(target_ee_right_pos, target_ee_orient)],
                [
                    (wrist_pos, None),
                    (waist_pos, None),
                    (elbow_pos, None),
            ...
            base_position = self.init_robot_pose(
                ...
                [
                    [(wrist_pos, None), (waist_pos, None)],
                    [(elbow_pos, None), (stomach_pos, None)],
                ],
                arm=["right", "left"],
