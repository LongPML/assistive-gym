...
class DressingEnv(AssistiveEnv):
    def __init__(self, robot, human_control=False):
        ...
        super(DressingEnv, self).__init__(
            robot=robot,
            ...
            time_step=0.02,
            obs_robot_len=29,
            obs_human_len=(25 if human_control else 0),
        )
        self.tt = None
    ...
    def step(self, action):
        ...
        for _ in range(self.frame_skip):
            ...
            ik_joint_poses = np.array(
                p.calculateInverseKinematics(
                    ...
                    targetOrientation=self.start_ee_orient,
            ...
            self.robot.set_joint_angles(
                self.robot.left_arm_joint_indices,
                target_joint_positions,
                use_limits=False,
            )

            # Force the end effector attachment to stay at the end effector
            self.end_effector_attachment.set_base_pos_orient(
                self.robot.get_pos_orient(self.robot.left_end_effector)[0], [0, 0, 0, 1]
            )
            ...
            forces_torques.append(
                self.end_effector_attachment.get_force_torque_sensor(0) - self.meanft
            )
        ...
        # Get cloth data
        x, y, z, cx, cy, cz, fx, fy, fz = p.getSoftBodyData(
            self.cloth, physicsClientId=self.id
        )
        ...
        (
            ...
            distance_along_upperarm,
            distance_to_hand,
            distance_to_elbow,
            distance_to_shoulder,
            forearm_length,
            upperarm_length,
        ) = self.util.sleeve_on_arm_reward(
            triangle1_points,
            triangle2_points,
            shoulder_pos,
            elbow_pos,
            wrist_pos,
            self.human.hand_radius,
            self.human.elbow_radius,
            self.human.shoulder_radius,
        )
        ...
        forces = (
            np.concatenate(
                [
                    np.expand_dims(fx, axis=-1),
                    np.expand_dims(fy, axis=-1),
                    np.expand_dims(fz, axis=-1),
                ],
                axis=-1,
            )
            * 10
        ...
        preferences_score = self.human_preferences(
            end_effector_velocity=end_effector_velocity, dressing_forces=forces
        )
        ...
        reward = (
            self.config("dressing_reward_weight") * reward_dressing
            + self.config("action_weight") * reward_action
            - 0.01 * np.linalg.norm(forces_ft)
            - 0.01 * np.linalg.norm(torques_ft)
            + preferences_score
        )
        ...
        obs = self._get_obs(
            np.concatenate([forces_ft, torques_ft]),
            [cloth_force_sum, robot_force_on_human],
        )
        ...
        if self.gui:
            ...
            print(
                "Force sensor:",
                np.linalg.norm(forces_ft),
                "Torque sensor",
                torques_ft,
                "Average forces on arm:",
                cloth_force_sum,
            )
        ...
        info = {
            ...
            "task_success": int(
                self.task_success >= self.config("task_success_threshold")
            ),
    ...
    def _get_obs(self, forces, forces_human):
        end_effector_pos, end_effector_orient = self.robot.get_pos_orient(
            self.robot.left_end_effector
        )
        (
            end_effector_pos_real,
            end_effector_orient_real,
        ) = self.robot.convert_to_realworld(end_effector_pos, end_effector_orient)
        ...
        if self.human_control:
            ...
            (
                end_effector_pos_human,
                end_effector_orient_human,
            ) = self.human.convert_to_realworld(end_effector_pos, end_effector_orient)
    ...
    def reset(self):
        ...
        self.wheelchair = self.world_creation.create_new_world(
        ...
        if self.robot.wheelchair_mounted:
            ...
            self.robot.set_base_pos_orient(
                wheelchair_pos + np.array(self.robot.toc_base_pos_offset[self.task]),
                [0, 0, -np.pi / 2.0],
                euler=True,
            )
        ...
        joints_positions = [
            ...
            (self.human.j_left_shoulder_x, -80),
        ...
        self.human.setup_joints(
            ...
            reactive_force=None if self.human_control else 1,
            reactive_gain=0.01,
        )
        ...
        target_ee_pos = np.array([0.45, -0.3, 1.2]) + self.np_random.uniform(
            -0.05, 0.05, size=3
        )
        target_ee_orient = np.array(
            p.getQuaternionFromEuler(
                np.array(self.robot.toc_ee_orient_rpy[self.task][0]),
                physicsClientId=self.id,
            )
        )
        ...
        if self.robot.wheelchair_mounted:
            ...
            target_ee_orient_shoulder = np.array(
                p.getQuaternionFromEuler(
                    np.array(self.robot.toc_ee_orient_rpy[self.task][1]),
                    physicsClientId=self.id,
                )
            )
            self.robot.position_robot_toc(
                ...
                [
                    (shoulder_pos + offset, target_ee_orient_shoulder),
                    (elbow_pos + offset, target_ee_orient),
                    (wrist_pos + offset, target_ee_orient),
                ],
        ...
        if self.human_control or self.human.impairment == "tremor":
            ...
            self.human.control(
                self.human.controllable_joint_indices,
                self.human.get_joint_angles(self.human.controllable_joint_indices),
                0.05,
                1,
            )

        self.start_ee_pos, self.start_ee_orient = self.robot.get_pos_orient(
            self.robot.left_end_effector
        )
        ...
        sphere_collision_1, sphere_visual_1 = self.world_creation.create_sphere(
            radius=0.0001,
            mass=0.1,
            pos=[0, 0, 0],
            visual=True,
            collision=False,
            rgba=[0, 0, 0, 0],
            return_collision_visual=True,
        )
        sphere_collision_2, sphere_visual_2 = self.world_creation.create_sphere(
            radius=0.0001,
            mass=0.1,
            pos=[0, 0, 0],
            visual=True,
        ...
        end_effector_attachment = p.createMultiBody(
            baseMass=0,
            baseCollisionShapeIndex=sphere_collision_1,
            baseVisualShapeIndex=sphere_visual_1,
            basePosition=self.start_ee_pos,
            baseOrientation=[0, 0, 0, 1],
            linkMasses=[0.1],
            linkCollisionShapeIndices=[sphere_collision_2],
            linkVisualShapeIndices=[sphere_visual_2],
            linkPositions=[[0, 0, 0]],
        ...
        self.end_effector_attachment.init(
            end_effector_attachment, self.id, self.np_random, indices=-1
        )

        self.cloth_attachment = self.world_creation.create_sphere(
            radius=0.0001,
            mass=1,
            pos=self.start_ee_pos,
            visual=True,
            collision=False,
            rgba=[0, 0, 0, 0],
        ...
        self.end_effector_attachment.create_constraint(
            0, self.cloth_attachment, self.cloth_attachment.base
        )
        ...
        self.cloth = p.loadCloth(
            os.path.join(
                self.world_creation.directory, "clothing", "hospitalgown_reduced.obj"
            ),
            scale=1.4,
            mass=0.16,
            position=np.array([0.02, -0.38, 0.83]) + self.cloth_offset / 1.4,
            orientation=p.getQuaternionFromEuler(
                [0, 0, np.pi], physicsClientId=self.id
            ),
            bodyAnchorId=self.cloth_attachment.body,
            anchors=[
                2087,
                3879,
                3681,
            ...
            rgbaColor=np.array([139.0 / 256.0, 195.0 / 256.0, 74.0 / 256.0, 0.6]),
            rgbaLineColor=np.array([197.0 / 256.0, 225.0 / 256.0, 165.0 / 256.0, 1]),
            physicsClientId=self.id,
        )
        ...
        p.setGravity(
            0, 0, -9.81 / 2, physicsClientId=self.id
        )  # Let the cloth settle more gently
        ...
        p.setGravity(
            0, 0, 0, body=self.end_effector_attachment.body, physicsClientId=self.id
        )
