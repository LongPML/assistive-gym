...
class FeedingEnv(AssistiveEnv):
    ...
    def step(self, action):
        ...
        (
            ...
            food_hit_human_reward,
        ) = self.get_food_rewards()
        ...
        info = {
            ...
            "task_success": int(
                self.task_success
                >= self.total_food_count * self.config("task_success_threshold")
            ),
    ...
    def _get_obs(self, agent=None):
        ...
        self.total_force_on_human = (
            self.robot_force_on_human + self.spoon_force_on_human
        )
        robot_obs = np.concatenate(
            [
                spoon_pos_real,
                spoon_orient_real,
                spoon_pos_real - target_pos_real,
                robot_joint_angles,
                head_pos_real,
                head_orient_real,
                [self.spoon_force_on_human],
            ]
        ).ravel()
        if agent == "robot":
            return robot_obs
        if self.human.controllable:
            ...
            human_obs = np.concatenate(
                [
                    spoon_pos_human,
                    spoon_orient_human,
                    spoon_pos_human - target_pos_human,
                    human_joint_angles,
                    head_pos_human,
                    head_orient_human,
                    [self.robot_force_on_human, self.spoon_force_on_human],
    ...
    def reset(self):
        ...
        target_ee_pos = np.array([-0.15, -0.65, 1.15]) + self.np_random.uniform(
            -0.05, 0.05, size=3
        )
