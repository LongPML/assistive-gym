...
def setup_config(env, algo, coop=False, seed=0, extra_configs={}):
    ...
    if coop:
        obs = env.reset()
        policies = {
            "robot": (None, env.observation_space_robot, env.action_space_robot, {}),
            "human": (None, env.observation_space_human, env.action_space_human, {}),
        }
...
def load_policy(
    env, algo, env_name, policy_path=None, coop=False, seed=0, extra_configs={}
):
    if algo == "ppo":
...
def train(
    ...
    agent, checkpoint_path = load_policy(
        env, algo, env_name, load_policy_path, coop, seed, extra_configs
    )
    if load_policy_path != "":
        checkpoint_num = int(load_policy_path.split("-")[-1])
        hist_stats_path = (
            "/".join(load_policy_path.split("/")[:-1])
            + f"/hist_stats_{checkpoint_num}.csv"
        )
        hist_stats = pd.read_csv(hist_stats_path)[
            [
                "episode",
                "episode_lengths",
                "episode_reward",
    ...
    while timesteps < timesteps_total:
        ...
        hist_stats["policy_human_reward"].extend(
            result["hist_stats"]["policy_human_reward"]
        )
        hist_stats["policy_robot_reward"].extend(
            result["hist_stats"]["policy_robot_reward"]
        )
        if coop:
        ...
        hist_stats_path = (
            ...
            + f"/hist_stats_{checkpoint_path.split('-')[-1]}.csv"
        )
...
def render_policy(
    ...
    test_agent, _ = load_policy(
        env, algo, env_name, policy_path, coop, seed, extra_configs
    )
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            if coop:
                ...
                action_robot = test_agent.compute_action(
                    obs["robot"], policy_id="robot"
                )
                action_human = test_agent.compute_action(
                    obs["human"], policy_id="human"
                )
                # Step the simulation forward using the actions from our trained policies
                obs, reward, done, info = env.step(
                    {"robot": action_robot, "human": action_human}
                )
                done = done["__all__"]
            else:
...
def evaluate_policy(
    ...
    for episode in range(n_episodes):
        ...
        if verbose:
            print(
                "Reward total: %.2f, mean force: %.2f, task success: %r"
                % (reward_total, np.mean(force_list), task_success)
            )
        sys.stdout.flush()
    env.disconnect()
...
def plot_hist_stats(hist_stats, title, episode_bin=20, label="PPO", coop=False):
    rewards_bin_mean = (
        ...
        .rolling(window=episode_bin)
        .mean()
        .values[episode_bin - 1 :: episode_bin]
    )
    rewards_bin_min = (
        hist_stats["episode_reward"]
        .rolling(window=episode_bin)
    ...
    steps_bin = [
        np.sum(hist_stats["episode_lengths"][: (i + 1) * 20]) for i in range(bin_length)
    ]
    ...
    plt.fill_between(
        steps_bin, rewards_bin_min, rewards_bin_max, color="blue", alpha=0.3
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env",
        default="ScratchItchJaco-v0",
        help="Environment to train on (default: ScratchItchJaco-v0)",
    )
    parser.add_argument(
        "--algo", default="ppo", help="Reinforcement learning algorithm"
    )
