...
human.setup_joints(
    joints_positions, use_static_joints=False, reactive_force=None, reactive_gain=0.05
)
# Set human base and increase body friction
human.set_base_pos_orient(
    [-2, 1, 0.89 if human.gender == "male" else 0.86], [-0.1, 0, -np.pi / 3.5]
)
...
robot.ik_random_restarts(
    right=True,
    target_pos=pos + np.array([-0.1, 0.25, 0.6]),
...
robot.ik_random_restarts(
    right=False,
    target_pos=pos + np.array([-0.15, 0.2, 0.6]),
    target_orient=orient + np.array([0, 0, -np.pi / 2.0]),
    step_sim=False,
    check_env_collisions=False,
)
