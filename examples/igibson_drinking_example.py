...
scene = BuildingScene(
    "Placida", is_interactive=True, build_graph=True, pybullet_load_texture=True
)
scene.load()
...
human.setup_joints(
    joints_positions, use_static_joints=False, reactive_force=10, reactive_gain=0.05
)
# Set human base and increase body friction
human.set_base_pos_orient(
    [-2.8, 1.125, 0.89 if human.gender == "male" else 0.86],
    [-0.1, 0, -np.pi / 3.5 - np.pi / 2.0 - np.pi / 6.0],
)
...
robot.ik_random_restarts(
    right=True,
    target_pos=pos + np.array([-0.4, 0.55, 0.85]),
...
robot.ik_random_restarts(
    right=False,
    target_pos=pos + np.array([-0.2, -0.05, 0.6]),
    target_orient=orient + np.array([0, 0, -np.pi / 2.0]),
    step_sim=False,
    check_env_collisions=False,
)
robot.set_gripper_open_position(
    robot.right_gripper_indices, [0.5] * 4, set_instantly=True
)
...
p.setPhysicsEngineParameter(
    numSubSteps=4, numSolverIterations=10, physicsClientId=env.id
)
...
target_pos, target_orient = p.multiplyTransforms(
    head_pos, head_orient, mouth_pos, [0, 0, 0, 1], physicsClientId=env.id
)
# target = env.create_sphere(radius=0.01, mass=0.0, pos=target_pos, collision=False, rgba=[0, 1, 0, 1])

target_joint_angles_cup = robot.ik(
    robot.right_end_effector,
    pos + np.array([-0.58, 0.55, 0.8]),
    orient,
    ik_indices=robot.right_arm_ik_indices,
    max_iterations=200,
)
target_joint_angles_lift = robot.ik(
    robot.right_end_effector,
    pos + np.array([-0.58, 0.55, 1.0]),
    orient,
    ik_indices=robot.right_arm_ik_indices,
    max_iterations=200,
)
...
for iteration in range(200):
    ...
    gains = 0.05
    if iteration < 20:
        actions[len(robot.wheel_joint_indices) :] = (
            target_joint_angles_cup - current_joint_angles
        )
    elif iteration < 25:
        robot.set_gripper_open_position(
            robot.right_gripper_indices, [0.4] * 4, set_instantly=False
        )
    elif iteration < 40:
        actions[len(robot.wheel_joint_indices) :] = (
            target_joint_angles_lift - current_joint_angles
        )
    elif iteration < 100:
        target_joint_angles_near_mouth = robot.ik(
            robot.right_end_effector,
            target_pos + mouth_offset,
            orient,
            ik_indices=robot.right_arm_ik_indices,
            max_iterations=200,
        )
        actions[len(robot.wheel_joint_indices) :] = (
            target_joint_angles_near_mouth - current_joint_angles
        )
        gains = 0.01
    else:
        target_joint_angles_tilt = robot.ik(
            robot.right_end_effector,
            target_pos + mouth_offset,
            orient + np.array([-np.pi / 1.8, 0, 0]),
            ik_indices=robot.right_arm_ik_indices,
            max_iterations=200,
        )
        actions[len(robot.wheel_joint_indices) :] = (
            target_joint_angles_tilt - current_joint_angles
        )
        gains = 0.01
    ...
    if iteration >= 25:
        robot.set_gripper_open_position(
            robot.right_gripper_indices, [0.4] * 4, set_instantly=True
        )
